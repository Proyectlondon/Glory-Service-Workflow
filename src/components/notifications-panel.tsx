...
import { ArrowLeft, Bell, CheckCircle2, ArrowRight, Download, Clock, Trash2, Info, Send, RotateCcw } from "lucide-react";
...
export function NotificationsPanel() {
  const { notifications, setNotifications, markNotificationsRead, setCurrentView, setSelectedWorkflowId } = useAppStore();
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((n, i) => (
                <motion.div key={n.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}>
                  <Card className={`cursor-pointer rounded-2xl border border-black/5 bg-white shadow-sm transition-all hover:shadow-md ${!n.read ? "ring-1 ring-[#007AFF]/20" : ""}`} onClick={() => openWorkflow(n.workflowId)}>
                    <CardContent className="flex items-start gap-3 p-4">
                      <div className="mt-0.5 flex h-9 w-9 items-center justify-center rounded-xl bg-[#F5F5F7]">{getIcon(n.type)}</div>
